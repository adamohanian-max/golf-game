...
--
--  SUPERSEDED (never had any client code) by match_chat below. A single
--  latest-message slot silently drops the first of two messages sent inside
--  one poll window and carries no history. Left in place rather than dropped
--  so re-running this file stays safe on a live database.
-- =====================================================================
...
-- =====================================================================
--  IN-ROUND CHAT (match_chat) — free-text messages between humans in a
--  match. Bot chat is entirely client-side (BOT_VOICES in game.js) and
--  never touches this table.
--
--  Rides the existing per-match realtime channel (subscribeMatchRealtime);
--  the 3s board poll re-fetches by created_at as a backstop, and the client
--  dedupes on row id so a message can arrive twice and render once.
--
--  MODERATION: none, by product decision — no filter, no report, no block,
--  and the 100-char CHECK plus permissive RLS below are the only limits.
--  That is acceptable while the game ships on the web, but it is NOT App
--  Store Guideline 1.2 compliant. Guideline 1.2 requires a content filter,
--  a report mechanism and user blocking for any app with user-generated
--  content. Add all three before the next iOS submission.
-- =====================================================================
...
create index if not exists match_chat_match_idx on match_chat (match_id, created_at desc);

alter table match_chat enable row level security;

-- Matches themselves are world-readable (see the note at "Hardening" above);
-- chat inside one is no more private than the scoreboard.
drop policy if exists "chat read" on match_chat;
create policy "chat read" on match_chat for select using (true);

-- Guests (user_id null) may post, exactly like the `rounds` insert policy —
-- a logged-in poster can only claim their own uid.
drop policy if exists "chat insert" on match_chat;
create policy "chat insert" on match_chat for insert
  with check (user_id is null or user_id = auth.uid());

alter table match_chat drop constraint if exists chat_body_len;
alter table match_chat add constraint chat_body_len
  check (char_length(body) between 1 and 100);
alter table match_chat drop constraint if exists chat_name_len;
alter table match_chat add constraint chat_name_len
  check (char_length(player_name) between 1 and 24);

-- Push instead of poll (the poll stays as a backstop).
do $$
begin
