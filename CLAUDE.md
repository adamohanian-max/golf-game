...
**Aerial surface mask (OOB + fairway/rough from what you SEE):** OSM polygons make OOB inconsistent (only `natural=wood` counted — neighborhoods/roads/parking defaulted to playable rough) and fairway/rough coarse. So the bake also classifies the aerial per-pixel into a coarse **label raster** (`courses/img/<id>/surfacemask.png`, palette `OB=0 / FAIRWAY=1 / ROUGH=2 / WOODS=3`) + a `surfaceMask:{file,w,h,toWorld}` affine. `build_surface_mask`/`_classify_px` in `fetch_course.py` (wired into `fetch_course_global.py` after the aerial downloads): green-turf by HSV (reuses `is_fairway_px`'s band); fairway-vs-rough by **brightness split** (Otsu over the green pixels → `_otsu`, mostly removes per-course tuning); woods = dark + high local edge energy (`FIND_EDGES`+`BoxBlur`); everything non-green is **OOB only outside the playing envelope** (buffered hole centerlines, `MASK_CORRIDOR_UNITS`) — inside it, non-turf (sand/path/dirt) is just rough, so a fairway bunker isn't mistaken for a parking lot. Despeckled (`ModeFilter`), OSM `woods` unioned in as a strong prior. Tunables: `MASK_*` constants. Pillow-optional + guarded (no Pillow / classify fail → no `surfaceMask` key). `--no-mask` skips it. **Game side:** `loadSurfaceMask`/`maskClassAt` (game.js) decode the PNG once (nearest-palette → `Uint8` labels) and sample it in `surfaceAt()` — OSM stays authoritative for green/bunker/water/tee, the mask drives fairway/rough/woods/`ob` (aerial overrides coarse OSM fairway/rough), falling back to OSM polygons on no-mask holes. New `"ob"` surface mirrors woods in `friction`/`bounce`/`lie`; OOB penalty (`woods`+`ob`) is gated by `TUNE.obPenalty` (default on = +1 & drop at `lastSafe`). `drawOOBOverlay` draws the OB+woods cells as a red tint through the aerial transform (gated by the `showOOB` button).
